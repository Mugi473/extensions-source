...
private fun fetchFilters(apiBaseUrl: String, client: OkHttpClient, headers: Headers) {
    ...
    thread {
        try {
            val response = client.newCall(GET("$apiBaseUrl/front/manga?page=1&limit=1", headers)).execute()
            val filters = response.parseAs<SearchResultsDto>()

            genresList = filters.genres.orEmpty().map { Tag(it.name, it.id) }
